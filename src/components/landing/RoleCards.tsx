...
const RoleCard = ({ role }: { role: (typeof roles)[0] }) => {
  ...
  return (
    <Link href={role.link} className="block h-full">
      <div className="bg-card rounded-xl shadow-md overflow-hidden border border-border/70 transition duration-300 ease-in-out hover:-translate-y-1 hover:shadow-xl h-full flex flex-col">
        <div className={`${colors.bg} p-6`}>
          <div className={`${colors.iconBg} w-14 h-14 rounded-full flex items-center justify-center mx-auto`}>
            <Icon className={`w-6 h-6 ${colors.iconText}`} />
          </div>
          <h3 className="mt-4 text-xl font-semibold text-center text-card-foreground">{role.name}</h3>
        </div>
        <div className="p-6 flex flex-col flex-grow">
          <ul className="space-y-3 flex-grow text-muted-foreground">
...
const RoleCards = () => {
  return (
    <section id="roles" className="py-16 px-4 sm:px-6 lg:px-8 bg-background">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-foreground mb-12">For Everyone in the Supply Chain</h2>
